...
//
    ...
    function showDate(yyyy,mm){
        ...
        for(var i=1;i<=daysCount;i++){
            ...
            var currentDate = i;
            if(i<10)
                currentDate = '0'+i.toString();

            currentDate = parseInt(activeYear+activeMonth+currentDate);

            var price_div= "<div class='f-yuan'></div>";
        ...
        for(var k=1; k<=35-(daysCount+monthstart);k++ ){//表格保持等高6行42个单元格
            mystr += "<div class='f-td f-nextMounth'>"+(k)+"</div>";
        }
