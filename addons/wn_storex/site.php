...
class Wn_storexModuleSite extends WeModuleSite {
	...
	public $_hotel_level_config = array(5 => '城市房源', 4 => '景区房源', 3 => '温泉房源', );
	...
	public function doMobiledetailed() {
		...
		$my_review = pdo_fetch("SELECT uid FROM " . tablename('storex_room_review') . " WHERE uid = :uid ", array(':uid' => $_W['fans']['uid']));
		//获取房态和放价
		//暂未开发
		
		include $this->template('detailed');
	}
