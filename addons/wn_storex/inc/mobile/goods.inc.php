...
$goodsid = intval($_GPC['goodsid']);//房源id
$max_room = 8;
...
if ($op == 'order') {
	...
	if ($goods_info['cprice'] == 0) {
		message(error(-1, '房源价格不能是0，请联系管理员!'), '', 'ajax');
	}
			...
			if ($flag == 1) {
				for($i = 0; $i < $days; $i++) {
					...
					if ($max_room == 0 || $max_room < $order_info['nums']) {
						message(error(-1, '房间数量不足,请选择其他房源或日期!'), '', 'ajax');
					}
				}
			}
