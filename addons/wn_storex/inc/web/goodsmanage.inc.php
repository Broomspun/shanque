...
$store_type = !empty($_GPC['store_type'])? intval($_GPC['store_type']) : 1;
...
if ($op == 'edit') {
	...
	if (checksubmit('submit')) {
		...
		message('房源信息更新成功！', $this->createWebUrl('goodsmanage', array('store_type' => $data['store_type'])), 'success');
	}
...
if ($op == 'display') {
	...
	$storex_bases = pdo_fetch("SELECT title FROM " . tablename('storex_bases') . " WHERE store_type = :store_type LIMIT 1", array(':store_type' => 1));
