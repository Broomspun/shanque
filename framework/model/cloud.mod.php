...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
	return true;
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars);
...
function cloud_build() {
	...
	if(!is_error($ret)) {
		...
		if($ret['family'] == 'x' && IMS_FAMILY == 'v') {
			...
			message('您已经获得山雀商业授权版本, 系统将转换为商业版, 并重新运行自动更新程序.', 'refresh');
		}
		$ret['upgrade'] = false;
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入山雀云服务黑名单，山雀云服务一切业务已被禁止，请联系山雀之家客服！');
	}
	if (strlen($dat['content']) != 32) {
		return error(-1, '山雀云服务平台向您的服务器传输数据过程中出现错误, 这个错误可能是由于您的通信密钥和山雀云服务不一致, 请尝试诊断山雀云服务参数(重置站点ID和通信密钥). 传输原始数据:' . $dat['meta']);
	}
	...
	if (empty($data) || empty($ret) || $dat['content'] != $ret['secret']) {
		return error(-1, '山雀云服务平台向您的服务器传输的数据校验失败, 可能是因为您的网络不稳定, 或网络不安全, 请稍后重试.');
	}
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars);
...
function cloud_cron_update($cron) {
	...
	$pars['cron'] = base64_encode(iserializer($cron));
	$result = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars);
	//http://v2.addonshanque.zhangshuoyin.cn/gateway.php
	return _cloud_cron_parse($result);
}
...
function _cloud_cron_parse($result) {
	...
	if (null === $result) {
		return error(-1, '山雀云服务通讯发生错误，请稍后重新尝试！');
	}
...
function cloud_auth_ip(){
	...
	$pars['cron_id'] = $cron_id;
	$pars['status'] = $status;
	$result = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php/a=ipcl', $pars);
	...
	$auth_url = 'http://shanque.zhangshuoyin.cn/gateway.php/a=ipcl&__auth=' . $query;
...
function cloud_auth_url($forward, $data = array()){
	...
	$query = base64_encode(json_encode($auth));
	$auth_url = 'http://shanque.zhangshuoyin.cn/gateway.php/a=passwort&__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://shanque.zhangshuoyin.cn/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
